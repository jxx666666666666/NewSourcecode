...
<template>
  <div class="loopChart">
    <div class="container">
      ...
      <div class="loop_label" v-if="textshow" :style="{ width: `${width}`, height: `${height}` }">
        <div
          class="text"
          :title="selectedSeries.legend"
          v-text="selectedSeries.legend"
          :style="{ fontSize: `${fontText}px` }"></div>
        <div class="num" :style="{ fontSize: `${fonts}px` }" v-text="selectedSeries.measure"></div>
    ...
    <div class="flex-ledengs" v-if="flexLegends == 1">
      <div class="legends-item" v-for="(item, index) in returnData" :key="index">
        <div class="left">
          <div class="name">{{ item.name }}</div>
          <div class="line">
            <div v-if="isGradual" :style="{ background: item.itemStyle.color, width: item.num }"></div>
            <div
              v-else
              :style="{
    ...
    <div class="flex-ledengs2" v-if="flexLegends == 2">
      <div class="legends-item" v-for="(item, index) in returnData" :key="index" @click="selectLegend(item)">
        <div class="legends">
          <div class="block" :style="{ background: item.itemStyle.color }"></div>
          <div class="name">{{ item.name }}</div>
          <div class="rate" style="height: 14px">
            ...
            <div
              :style="{
                'background-image':
                  'linear-gradient(to right, ' + item.itemStyle.color + '00' + ',' + item.itemStyle.color + 'ff' + ')',
    ...
    <div class="flex-ledengs4" v-if="flexLegends == 4">
      <div class="legend">
        <div class="legend_item" v-for="(item, index) in returnData" :key="index">
...
export default {
  ...
  props: {
    ...
    rowData: {
      ...
      default: () => {
        return [
          { legend: '360 ° Patrol Vehicle Calls', measure: 100, rate: '13.33%' },
          { legend: 'Number of drone calls', measure: 100, rate: '13.33%' },
          { legend: 'Number of law enforcement instrument calls', measure: 100, rate: '13.33%' },
          { legend: 'Number of emergency firefighting equipment calls', measure: 100, rate: '13.33%' },
        ];
      },
    },
    rowDatas: {
    ...
    legends: {
      type: Array,
      default: () => {
        return ['#2872EC', '#EBCF57', '#00F4EA', '#00DD8A', '#CDFC2E', '#BFFF00', '#00EADE', '#00C7EA', '#1180EE'];
      },
    },
    //Legend data
    Graduallegends: {
  ...
  computed: {
    ...
    progressNum() {
      return function (aaa) {
        return ((aaa / this.allNum) * 100).toFixed(1) + '%';
      };
    },
  },
  methods: {
    ...
    convertData(rowData, legends) {
      ...
      if (rowData.length > 0) {
        ...
        returnData.forEach((d) => {
          d.num =
            this.sum == 0 ? '0' : Math.round((d.value / this.sum) * 100 * Math.pow(10, 1)) / Math.pow(10, 1) + '%';
        });
    ...
    lengedCarousel() {
      ...
      if (that.rowData.length > 10) {
        let Scroll = function () {
          ...
          let goTop = parseInt(countHeight.scrollHeight - countHeight.offsetHeight);
